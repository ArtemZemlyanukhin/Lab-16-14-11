...
int inputEmployee(struct Employee* emp) {
    ...
    printf("Фамилия: ");
    if (scanf("%s", emp->lastName) != 1) return 0;

    printf("Имя: ");
    if (scanf("%s", emp->firstName) != 1) return 0;

    printf("Отчество: ");
    if (scanf("%s", emp->middleName) != 1) return 0;

    printf("Должность: ");
    if (scanf("%s", emp->position) != 1) return 0;

    printf("Зарплата: ");
    if (scanf("%lf", &emp->salary) != 1) return 0;

    printf("Дата рождения (день месяц год): ");
    if (scanf("%d %d %d", &emp->birthDate.day,
        &emp->birthDate.month,
        &emp->birthDate.year) != 3) return 0;
...
int findEmployeesInMay(struct Employee employees[], int count) {
    ...
    for (int i = 0; i < count; i++) {
        if (employees[i].birthDate.month == 5) {
...
int append_into_file(char* fname, struct Employee employees[], int cnt) {
    ...
    for (int i = 0; i < cnt; i++) {
        if (employees[i].birthDate.month == 5) {
            may_count++;
            fprintf(out, "%d. %s %s %s - %s (%.2lf руб.) - %02d.%02d.%d\n",
                may_count,
